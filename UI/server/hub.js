...
class TelemetryHub extends EventEmitter {
  constructor({ io, cfg, watchdog, positioning, reports }) {
    super();
    this.io = io; this.cfg = cfg;
    this.watchdog = watchdog; this.positioning = positioning; this.reports = reports;
  ...
  ingest(packet) {
    ...
    if (packet.triage_tier === 'red' && prevTier !== 'red') {
      const inc = this.reports.record({
        worker_id: packet.worker_id, domain: packet.domain, type: 'casualty',
        tier: 'red', reasons: packet._triage_reasons || packet.reasons || [], ts: packet.ts,
      });
  ...
  onTransition(t) {
    ...
    if (t.to === 'lost') {
      const inc = this.reports.record({
        worker_id: t.worker_id, domain: last && last.domain, type: 'signal_lost',
        ts: Math.floor(Date.now() / 1000),
      });
      this.emit('incident', inc);
    } else if (t.to === 'ok' && t.from === 'lost') {
      this.reports.record({
        worker_id: t.worker_id, domain: last && last.domain, type: 'recovered',
        ts: Math.floor(Date.now() / 1000),
      });
    }
  ...
  workers() {
    return Array.from(this.last.values()).map((p) => ({
      worker_id: p.worker_id, domain: p.domain, triage_tier: p.triage_tier,
      comms_status: this.watchdog.status(p.worker_id), last_ts: p.ts,
    }));
  }
}
